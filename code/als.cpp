...
double predict_credit(double *user,double *item,int itemNum,int k,int i,int j){
	double result=0.0;
	double *userPoint=user+i*k;
	double *itemPoint=item+j;
	for(int z=0;z<k;++z){
		result+=(*userPoint)*(*itemPoint);
		++userPoint;
		itemPoint+=itemNum;
	}
	return result;
}
double als_obj(const double *user,const double *item,int k,double *g,const Probelm &data,bool fixUser){
	if(0==g){
	...
	int userNum=data.l;
	int itemNum=data.n;
	if(fixUser)	memset(g,0,sizeof(g)*itemNum*k);
	else	memset(g,0,sizeof(g)*userNum*k);
